...
class BlockShape {
    constructor(blockTexture, shapeInfo, rotation, column) {
        this.shapeInfo = shapeInfo;
        const color = Math.random() * 16777215;
        let adjustedPositions = shapeInfo.shape;
    ...
    _drawBlocks(rowsCols, blockTexture, color) {
        const blocks = [];
        for (let row = 0; row < rowsCols.length; row++) {
            const rowBlocks = [];
            for (let col = 0; col < rowsCols[row].length; col++) {
                if (rowsCols[row][col] !== 0) {
                    const block = new Block_1.Block(blockTexture, this);
                    block.x = col * consts_1.consts.BlockWidth;
                    block.y = row * consts_1.consts.BlockHeight;
    ...
    _changeStartColumn(positions, column) {
        const newPositions = [];
        for (let y = 0; y < positions.length; y++) {
            const newRow = _.fill(Array(positions[y].length), 0);
        ...
        for (let y = 0; y < shape.length; y++) {
            for (let x = 0; x < shape[y].length; x++) {
                if (shape[y][x] !== 0) {
                    const newX = -y;
                    const newY = x;
                    newPoints.push(new pixi_js_1.Point(newX, newY));
                }
            }
        }
        let lowestX = 0;
...
const BlockShape_1 = __webpack_require__(/*! ./BlockShape */ "./src/BlockShape.ts");
const consts_1 = __webpack_require__(/*! ./consts */ "./src/consts.ts");
class Game extends PIXI.Container {
    constructor() {
        super();
        this._score = 0;
        this._spawnedShapes = 0;
        this._init()
            .then(() => this._start());
    }
    _init() {
        return __awaiter(this, void 0, void 0, function* () {
            this._grid = new Grid_1.Grid(3, 12);
            this.addChild(this._grid);
            const scoreTitle = new PIXI.Text("Score:");
            this._scoreText = new PIXI.Text("");
            this._scoreText.anchor.x = 1;
            scoreTitle.x = 0;
            this._scoreText.x = consts_1.consts.GameWidth;
            this.addChild(scoreTitle);
            this.addChild(this._scoreText);
            this._data = yield this._fetchDataJson();
            this._graphics = yield this._fetchGraphics();
        });
    }
    _start() {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    _dropShape() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            while (this._grid.findFullRow() > -1) {
                ...
                this._score += 10;
                this._refreshScoreText();
                yield Utils_1.utils.timeout(500);
            }
            this._dropShape();
        });
    }
    ...
    _spawnShape() {
        const outcome = this._data.outcome[this._spawnedShapes];
        const outcomeShapeInfo = this._findShape(outcome);
        const texture = this._graphics["baseshape"].texture;
        this._spawnedShapes++;
        return new BlockShape_1.BlockShape(texture, outcomeShapeInfo, outcome.rotation, outcome.column);
    }
    _findShape(outcome) {
        for (const shape of this._data.shapes) {
            if (outcome.name === shape.name)
                return shape;
        }
    }
    _fetchDataJson() {
...
class Grid extends PIXI.Container {
    ...
    addShape(shape) {
        ...
        for (let row = 0; row < shape.blocks.length; row++) {
            for (let col = 0; col < shape.blocks[row].length; col++) {
                const block = shape.blocks[row][col];
                if (block !== null) {
                    this.addChild(block);
        ...
        for (let row = this._gridArray.length - 1; row > -1; row--) {
            for (let col = this._gridArray[row].length - 1; col > -1; col--) {
                if (this._gridArray[row][col] !== null && shape.contains(this._gridArray[row][col])) {
                    this._gridArray[row][col].y += consts_1.consts.BlockHeight;
    ...
    canMoveDownShape(shape) {
        for (let row = this._gridArray.length - 1; row > -1; row--) {
            for (let col = 0; col < this._gridArray[row].length; col++) {
                const cell = this._gridArray[row][col];
                if (shape.contains(cell)) {
                    const outOfBounds = this._gridArray.length < (row + 2);
                    if (outOfBounds)
                        return false;
                    const nextRowHasBlock = this._gridArray[row + 1][col] !== null;
                    const shapeContainsNextRow = shape.contains(this._gridArray[row + 1][col]);
    ...
    findFullRow() {
        for (let row = this._gridArray.length - 1; row > -1; row--) {
            let blocksInRow = 0;
            for (let col = 0; col < this._gridArray[row].length; col++) {
                if (this._gridArray[row][col] !== null)
                    blocksInRow++;
            }
            if (blocksInRow === this._gridArray[row].length) {
                return row;
            }
        }
        return -1;
...
(() => __awaiter(void 0, void 0, void 0, function* () {
    ...
    app.stage.addChild(new Game_1.Game());
